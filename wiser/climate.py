...
HOTWATER_ENTITYID = "sensor.wiser_hot_water"
...
async def async_setup_entry(hass, config_entry, async_add_entities):
    ...
    def get_schedule(service):
        ...
        if entity_id == HOTWATER_ENTITYID:
            schedule_data = data.wiserhub.getHotwaterSchedule()
            _LOGGER.debug("Sched Service Data = %s", schedule_data)
            if schedule_data is not None:
                schedule_data = convert_from_wiser_schedule(schedule_data, "Hotwater")
                yaml.save_yaml(filename, schedule_data)
            else:
                raise Exception("No schedule data returned")
    ...
    def set_schedule(service):
        ...
        if entity_id == HOTWATER_ENTITYID:
            hass.async_create_task(room.set_hotwater_schedule(schedule_data))
...
class WiserRoom(ClimateEntity):
    ...
    async def set_hotwater_schedule(self, schedule_data):
        """Set room schedules."""
        if schedule_data is not None:
            schedule_data = convert_to_wiser_schedule(schedule_data, "HotWater")
            await self.hass.async_add_executor_job(
                partial(self.data.wiserhub.setHotwaterSchedule, schedule_data)
            )
            _LOGGER.debug("Set hotwater schedule")
            self._force_update = True
            await self.async_update_ha_state(True)
            return True
        return False
