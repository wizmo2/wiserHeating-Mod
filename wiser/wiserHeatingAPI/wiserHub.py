...
class wiserHub:
    ...
    def getHotwater(self):
        ...
        self.checkHubData()
        try:
            return self.wiserHubData.get("HotWater")[0]
        except:
            return None
    ...
    def getRoomSchedule(self, roomId):
        ...
        if roomId == HOTWATER_SCHEDULE or self.getRoom(roomId) is None:
    ...
    def getHotwaterSchedule(self):
        """
        Gets Hotwater Schedule Data

        return: json data
        """
        self.checkHubData()

        scheduleId = self.getHotwater().get("ScheduleId")
        if scheduleId is not None:
            for schedule in self.wiserHubData.get("Schedule"):
                if schedule.get("id") == scheduleId:
                    return schedule
            raise WiserNotFound("Hotwater Schedule not found ")
        else:
            raise WiserNotFound("Hotwater not found ")

    def setHotwaterSchedule(self, scheduleData: dict):
        """
        Sets Hotwater Schedule
        ...
        if scheduleId is not None:
            ...
            if response.status_code != 200:
                ...
                raise WiserRESTException(
                    "Error setting schedule for Hotwater, error {} {}".format(
                        response.status_code, response.text
                    )
                )
        else:
            raise WiserNotFound("No schedule found that matches Hotwater")

    def setHotwaterScheduleFromFile(self, scheduleFile: str):
        """
        Sets Hotwater Schedule
